...
scythe_adapters_p1_path="./refs/truseq_illumina_3p_adapt_R1.fa"
scythe_adapters_p2_path="./refs/truseq_illumina_3p_adapt_R2.fa"

# refgff - caminho para o arquivo contendo as coordenadas gênicas
refgff="./U_gibba/Utricularia_gibba_annos1-cds0-id_typename-nu1-upa1-add_chr0.gid25895.gff"

# refbt2idx - caminho para a base com o índice do genoma para o bowtie2
refbt2idx="./U_gibba/U_gibba_genome_fixed"

# regenomefa - caminho para o arquivo com as sequências do genoma no formato multifasta
refgenomefa="./U_gibba/U_gibba_genome_fixed.fa"

# tophat_transcriptome_index - bowtie2 transcriptome index relacionado ao genes.gtf selecionado
tophat_transcriptome_index="./refs/transcriptome_index/Ugib-transcriptome_index/"

# cutadapt_5p_adapter - adaptador 3' para ser usado com cutadapt
cutadapt_adapters_p1_path="./refs/truseq_illumina_5p_adapt_R1.fa"
cutadapt_adapters_p2_path="./refs/truseq_illumina_5p_adapt_R2.fa"
...
for bs in ${biosamps[@]}; do
	...
	then
		...
		then
			...
			cat ${input}/${bs}_T*_R1.fastq | fastx_trimmer -l ${min} -Q 33 > ${fastx_out}/${bs}_1.fastq
		else
	...
	then
		...
		techreps=(`ls ${input}/${bs}_T*_R2.fastq`)
		if ((${#techreps[@]} > 1))
		then
			echo "* Trimming ${bs} by minimum length (${min}) and merging ${#techreps[@]} technical replicates of ${bs} [R2] ..."
			cat ${input}/${bs}_T*_R2.fastq | fastx_trimmer -l ${min} -Q 33 > ${fastx_out}/${bs}_2.fastq
		else
...
for fastq in `ls ${fastx_out}/*_1.fastq`; do
	...
	then
		...
		cufflinks --num-threads ${num_threads} --library-type fr-firststrand --multi-read-correct --GTF-guide ${refgff} --frag-bias-correct ${refgenomefa} --total-hits-norm --output-dir ${cufflinks_sample_out} ${tophat_sample_out}/accepted_hits.bam --max-bundle-frags 2000000 &> ${cufflinks_sample_out}/${fqname}.cufflinks.log.txt
...
for bg in ${biogroups[@]}; do
       	echo "	${bg} ..."	
	bamlist=(`find ${tophat_out}/ -name accepted_hits.bam | grep "${bg}" | sort`)
	# aqui temos que usar o asterisco ("*") para expandir o conteúdo de bamlist pois queremos apenas uma palavra, separada pela variável especial IFS e não múltiplas palavras como quando usamos o arroba ("@")

	biogroupsbam=(${biogroupsbam[@]} $(IFS=, ; echo "${bamlist[*]}"))
done;
...
then
	...
	cuffdiff --num-threads ${num_threads} --library-type fr-firststrand --multi-read-correct --frag-bias-correct ${refgenomefa} --library-norm-method geometric --compatible-hits-norm --output-dir ${cuffdiff_out} ${cuffmerge_out}/merged.gtf ${biogroupsbam[@]} --FDR  0.05 --labels $(IFS=, ; echo "${biogroups[*]}") --max-bundle-frags 999999999 --frag-len-mean 500 --frag-len-std-dev 100 &> ${cuffdiff_out}/cuffdiff.log
